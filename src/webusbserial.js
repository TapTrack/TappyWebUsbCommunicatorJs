(function (root, factory) {
    ...
    const FTDI_USB_VENDOR_ID = 0x0403;
    const FTDI_USB_PRODUCT_ID = 0x6001;
    const NATIVE_USB_VENDOR_ID = 0x04d8;
    const NATIVE_USB_PRODUCT_ID = 0x0053;

    const DEFAULT_FILTERS = [
        { 'vendorId':FTDI_USB_VENDOR_ID, 'productId': FTDI_USB_PRODUCT_ID }, // TapTrack TappyUSB
        { 'vendorId':NATIVE_USB_VENDOR_ID, 'productId': NATIVE_USB_PRODUCT_ID }, // TapTrack TappyUSB
    ];
    ...
    FTDIReadHead.prototype.readLoop = function() {
        ...
        if (!self.cancelled) {
            self.device.transferIn(1, 64).then(function(result){
                ...
                setTimeout(() => {self.readLoop();},arr.length === 2 ? 50 : 0);
            }, function(error){
                if (!self.cancelled) {
    ...
    function WebFTDIUSBSerialPort(params) {
        ...
        if (typeof params !== "undefined" &&
            params !== null &&
            typeof params.debug === "boolean" && params.debug) {
    ...
    WebFTDIUSBSerialPort.prototype.emit = function(channel, data) {
        var self = this;
        if (typeof self.listeners[channel] === "object") {
    ...
    WebFTDIUSBSerialPort.prototype.open = function (callback) {
        ...
        if (self.isChangingConfiguration) {
            let err = new Error("already changing device configuration");
            self.emit('error', err);
            callback(err);
        } else if (!self.isOpen) {
            if (typeof self.device !== "undefined" && self.device !== null) {
                ...
                self.device.open()
                    ...
                    .then((s)=>{
                        self.debugLog("setting data bits");
                        //data bits
                    ...
                    .then((s)=>{
                        self.debugLog("setting parity setting");
                        // parity off
                    ...
                    .then((s)=>{
                        self.debugLog("setting stop bit setting");
                        // stop bits
                ...
                let err = new Error("no device");
                self.emit('error', err);
                callback(err);
            }
        } else {
            let err = new Error("serial port already open");
            self.emit('error', err);
            callback(err);
        }
    };
    ...
    WebFTDIUSBSerialPort.prototype.write = function (data, callback) {
        var self = this;
        self.device.transferOut(2, data)
            .then(function(result){
                if(callback){
                    callback(null);
                }
    ...
    WebFTDIUSBSerialPort.prototype.close = function (callback) {
        var self = this;
        if (self.isChangingConfiguration) {
            let err = new Error("device configuration is currently changing");
            self.emit('error', err);
            callback(err);
        } else if (self.isOpen) {
            self.isChangingConfiguration = true;
            ...
            let err = new Error("serial port is not open");
            self.emit('error', err);
            callback(err);
        }
    };
    ...
    NativeUSBReadHead.prototype.readLoop = function() {
        ...
        if (!self.cancelled) {
            self.device.transferIn(1, 64).then(function(result){
                ...
                setTimeout(() => {self.readLoop();},arr.length > 0 ? 0 : 5);
            }, function(error){
                if (!self.cancelled) {
    ...
    WebNativeUSBSerialPort.prototype.write = function (data, callback) {
        var self = this;
        self.device.transferOut(1, data)
            .then(function(result){
                if(callback){
                    callback(null);
                }
    ...
    WebNativeUSBSerialPort.prototype.close = function (callback) {
        ...
        if (self.isChangingConfiguration) {
            ...
            self.device.releaseInterface(0).then( () => {
